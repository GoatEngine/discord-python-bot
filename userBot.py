...
async def on_message(message):
    ...
    if message.author.id != '146068146476810240':
        return 
    ...
    if message.content.lower().startswith('>clap'):
        ...
        m = n.replace(" ", " :clap: ")
        print(m)
        await client.edit_message(message,m)
    if message.content.lower().startswith('>do'):
        n = message.content[3:]
        try:
            print(eval(n))
            await client.edit_message(message,eval(n))
        except:
                print("sorry we couldn't >do that")

#    if message.content.lower().startswith('>emo'):
#        n = ''
...
client.run("nick.strick32@gmail.com", "FireWolf471")
logs.close()
